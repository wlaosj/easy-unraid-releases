...
1. 填入您的 Unraid API 链接及 SSH 端口。
2. 在 App 配置中生成或填入您已有的 SSH 密钥对，并复制 App 展现 of **公钥 (Public Key)**。
...
> * **Flexible Options**: Unraid natively includes **built-in WireGuard support**. Additionally, users can easily deploy **Tailscale** or **ZeroTier**異地组网 from the Community Applications store with one click.
